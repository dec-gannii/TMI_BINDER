...
//            self.db.collection("Schedule").document(Auth.auth().currentUser!.uid).collection(self.date).document(self.editingTitle)
            self.db.collection("teacher").document(Auth.auth().currentUser!.uid).collection("schedule").document(self.date).collection("scheduleList").document(self.editingTitle).getDocument { (document, error) in
...
//            self.db.collection("Schedule").document(Auth.auth().currentUser!.uid).collection(self.date).document(editingTitle)
            self.db.collection("teacher").document(Auth.auth().currentUser!.uid).collection("schedule").document(self.date).collection("scheduleList").document(self.editingTitle).delete() { err in
                if let err = err {
...
//            self.db.collection("Schedule").document(Auth.auth().currentUser!.uid).collection(date).document(scheduleTitle.text!)
            self.db.collection("teacher").document(Auth.auth().currentUser!.uid).collection("schedule").document(self.date).collection("scheduleList").document(scheduleTitle.text!).setData([
...
//                    self.db.collection("Schedule").document(Auth.auth().currentUser!.uid).collection(date)
                    self.db.collection("teacher").document(Auth.auth().currentUser!.uid).collection("schedule").document(self.date).collection("scheduleList").getDocuments()
                    {
                        ...
                        else
                        {
                            ...
                            if (count == 1) {
                                // 1으로 저장
//                                self.db.collection("Schedule").document(Auth.auth().currentUser!.uid).collection(self.date).document("Count")
                                self.db.collection("teacher").document(Auth.auth().currentUser!.uid).collection("schedule").document(self.date).collection("scheduleList").document("Count").setData(["count": count])
                                { err in
                                    if let err = err {
                                        print("Error adding document: \(err)")
                                    }
